#!/usr/bin/env python3
"""
Automated Resume Tailor V2 - ALTERNATIVE LOGIC

DIFFERENCES FROM V1:
- Extracts 30 keywords (instead of 15) for comprehensive ATS coverage
- Work Experience section comes FIRST (before Projects and Education)
- All bullet points are crafted fresh based on job description keywords
- Avoids repeating existing work experience content verbatim
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('resume_tailor_v2.log'),
...
class ResumeTailor:
    ...
    def __init__(self, dry_run: bool = False):
        """Initialize the ResumeTailor V2 with alternative logic."""
        ...
        logger.info(f"ResumeTailor V2 initialized - Dry run: {self.dry_run}, LLM: {self.llm_provider}, Logic: 30 keywords, Experience-first")
            ...
            for r in range(header_row_idx0 + 1, len(values)):
                ...
                if rec.get('KEYWORDS') and not overwrite_keywords:
                    ...
                    if not job_title or not job_description:
                        ...
                        logger.info(f"Row {rec['__ROW_INDEX']}: Extracting 30 keywords for '{job_title}' (V2 logic)")
                        # V2 always extracts 30 keywords
    ...
    def _extract_keywords(self, job_description: str) -> List[Dict[str, Any]]:
        ...
        if self.dry_run:
            logger.info("DRY RUN: Would extract 30 keywords (V2 logic)")
            return [
                ...
                {"term": "aws", "rank": 21}, {"term": "kubernetes", "rank": 22},
                {"term": "ci/cd", "rank": 23}, {"term": "microservices", "rank": 24},
                {"term": "postgresql", "rank": 25}, {"term": "mongodb", "rank": 26},
                {"term": "redis", "rank": 27}, {"term": "elasticsearch", "rank": 28},
                {"term": "kafka", "rank": 29}, {"term": "spark", "rank": 30}
            ]
...
- Focus on: technical skills, software, tools, programming languages, frameworks, methodologies, certifications, domain knowledge
- Rank 1 = highest importance, 30 = lowest importance
- Use lowercase only
...
OUTPUT: JSON only, no explanations."""
        ...
        user_prompt = f"""Extract and rank the 30 most important keywords for ATS optimization from this job description:

{job_description}

Focus on technical requirements, required skills, preferred skills, and specific tools/technologies mentioned. Include both must-have and nice-to-have skills."""
        ...
        try:
            ...
            if json_match:
                ...
                logger.info(f"Extracted {len(keywords)} keywords (V2 - comprehensive mode with 30 keywords)")
                return keywords
            else:
...
**V2 LOGIC - KEY DIFFERENCES:**
- Professional Experience section comes FIRST (most important for recruiters)
- All bullet points crafted fresh using job description keywords
- Avoid repeating generic statements; make every bullet unique and tailored
- Use all 30 extracted keywords strategically throughout the experience
...
• [Generate BRAND NEW tailored 2-line bullet using ONLY keywords from the job description - focus on different aspects than UH role]
• [Generate BRAND NEW tailored 2-line bullet using ONLY keywords from the job description - focus on different aspects than previous bullet]
• [Generate BRAND NEW tailored 2-line bullet using ONLY keywords from the job description - focus on different aspects than previous bullets]

A. V.C Pvt Ltd. | {industrial_job_title} | TN, India | January 2021 -- May 2022
• [Generate BRAND NEW tailored 2-line bullet using ONLY keywords from the job description - focus on unique achievements]
• [Generate BRAND NEW tailored 2-line bullet using ONLY keywords from the job description - different from all previous bullets]
• [Generate BRAND NEW tailored 2-line bullet using ONLY keywords from the job description - unique accomplishments only]

===PROJECTS===
Quality 4.0: Digital Twin with LLM-Powered SPC Analytics | University of Houston | Feb 2025 - Present
• [Strong bullet point following STAR method with quantified results - focus on simulation and LLM integration]
• [Strong bullet point following STAR method with quantified results - focus on Unity and real-time monitoring]

Time Series Predictive Analytics-Operational Performance | University of Houston | Jan 2024 - May 2024
• [Strong bullet point following STAR method with quantified results - focus on predictive analytics]
• [Strong bullet point following STAR method with quantified results - focus on operational optimization]
...
**BULLET POINT REQUIREMENTS (V2 LOGIC):**

**CRITICAL V2 REQUIREMENT - KEYWORD-DRIVEN BULLETS:**
- You have 30 keywords extracted from the job description
- EVERY bullet point for IIT Madras and A.V.C Pvt Ltd MUST incorporate at least 3-5 of these keywords
- DO NOT repeat the same keywords across all bullets - distribute them strategically
- Each bullet should focus on DIFFERENT aspects of the role using DIFFERENT keywords
- Make bullets feel authentic to the job description, not generic
- Example: If keywords include "automation," "scada," "plc programming," "oee," use them naturally in bullets about those specific technologies
...
- **V2 CRITICAL**: For Centre for Automotive Energy Materials at IIT Madras: create EXACTLY 3 bullet points (each 180-200 chars, full 2 lines) - MUST use keywords from job description, avoid generic statements, make each bullet focus on DIFFERENT aspects
- **V2 CRITICAL**: For A.V.C Pvt Ltd: create EXACTLY 3 bullet points (each 180-200 chars, full 2 lines) - MUST use DIFFERENT keywords than IIT Madras bullets, avoid overlap, ensure variety
- **V2 CRITICAL**: NO REPETITION - Each of the 6 work experience bullets (3 for IIT Madras + 3 for A.V.C) must be UNIQUE. Do not reuse the same keywords or accomplishments across bullets
...
- **NO DUPLICATE SKILLS** across all 6 skill categories three bullet points each one 8200 characters must use keyword from job description that is correct
...
Return ONLY the structured text with ===SECTION=== markers above. Do not include any formatting specifications or instructions in your response."""
        ...
        keywords_str = '; '.join([kw['term'] for kw in sorted(keywords, key=lambda x: x['rank'])[:30]])  # All 30 keywords for V2
...
### ALL 30 KEYWORDS (V2 - USE STRATEGICALLY ACROSS ALL BULLETS):
{keywords_str}

### SPECIFIC INSTRUCTIONS FOR THIS ROLE (V2 LOGIC):
...
2. **V2 BULLET POINT STRATEGY - KEYWORD DISTRIBUTION**:
   For IIT Madras (3 bullets):
   - Bullet 1: Use keywords ranked 1-10 (highest priority skills)
   - Bullet 2: Use keywords ranked 11-20 (mid-priority skills)
   - Bullet 3: Use keywords ranked 21-30 (secondary skills)

   For A.V.C Pvt Ltd (3 bullets):
   - Bullet 1: Use DIFFERENT keywords than IIT Madras Bullet 1 (e.g., if IIT used "automation," use "data analysis")
   - Bullet 2: Use DIFFERENT keywords than IIT Madras Bullet 2 (ensure no overlap)
   - Bullet 3: Use DIFFERENT keywords than IIT Madras Bullet 3 (maximize variety)
   ...
   **Each bullet must**:
   ...
   - Be completely unique (no repetition across all 6 bullets)
...
### TRANSFORMATION APPROACH (V2 - KEYWORD-FIRST STRATEGY):
- Start with the 30 keywords - these are the foundation of every bullet
- For EACH bullet, select 3-5 keywords that fit together logically
- Craft a compelling achievement story around those specific keywords
- Use the same companies and timeframes but create BRAND NEW achievements based on keywords
- DO NOT copy/paste existing work experience - generate fresh content using keywords as inspiration
- Each bullet should feel like it was written specifically for THIS job
- Ensure ZERO overlap between bullets - if one bullet covers "automation," the next should cover "data analysis" or "quality control"
